        ...
        def _arguments_wrapper(request, *args, **kwargs):
            ...
            if not current_role in roles:
                if request.is_ajax():
                    ...
                    return render(request, 'users/errors/403.html', context)
...
def is_user_banned(function):
    def wrap(request, *args, **kwargs):
        if Profile.objects.filter(user=request.user).exists():
            profile = Profile.objects.get(user=request.user)

            if profile.is_banned:
                return HttpResponseRedirect(reverse("users:is_user_banned"))
...
def check_profile_status(function):
    def wrap(request, *args, **kwargs):
        if request.user.is_authenticated and not request.user.is_superuser:
            ...
            if current_role == 'employee':
                ...
                if not companyModel.Profile.objects.filter(user=request.user).exists():
                    return HttpResponseRedirect(reverse("company:my_profile"))
