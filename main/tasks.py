...
def product_expiry_reminder_task(text):
    ...
    # instances = ProductExpiryDate.objects.filter(
    #     is_deleted=False, expiry_date__gte=today)
    # for instance in instances:
    #     if instance.product.product_expiry_before:
    #         to_date = today + \
    #             datetime.timedelta(days=instance.product.product_expiry_before)
    #         if to_date <= instance.expiry_date:
    #             subject = NotificationSubject.objects.get(
    #                 code="expiry_notification")
    #             for user in superusers:
    #                 Notification(user=user, subject=subject, product=instance.product,
    #                              shop=instance.shop, is_active=True, expiry_date=instance).save()
    #             staffs = Staff.objects.filter(is_deleted=False)
    #             for staff in staffs:
    #                 user = staff.user
    #                 if staff.permissionlist:
    #                     permlist = [str(x.code)
    #                                 for x in staff.permissions.all()]
